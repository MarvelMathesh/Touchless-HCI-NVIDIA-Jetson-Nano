...
Implements weighted consensus voting with recency bias.
"""
...
class TemporalFilter:
    ...
    def update(self, gesture_result) -> dict:
        ...
        for i, (g, c, t) in enumerate(self._window):
            ...
            recency = 1.0 + self._recency_weight * i
            weight = c * recency
            votes[g] = votes.get(g, 0.0) + weight
            total_weight += weight

        if total_weight == 0:
            return {"gesture": None, "confidence": 0.0, "stable": False, "raw": gesture_result}
        ...
        if best_ratio < self._consensus_ratio:
            # No consensus - reset tracking
            self._current_gesture = None
            self._gesture_onset = None
            return {"gesture": None, "confidence": best_ratio, "stable": False, "raw": gesture_result}
        ...
        from modules.recognition.gesture_classifier import GestureType
        # Don't emit "none"
